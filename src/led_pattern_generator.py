...
and fade time parameters. LED machine steps through the table rows and holds then
fades for the given times. Pattern starts after GO command is sent.
...
TERM = 0xDE
LED_CMD = [0x01, 0xAA]  # Partial command.  We must fill in the leds, fade, hold and TERM
GO_CMD = [0x02, TERM]   # Start from current row (for seamless animation)
VER_CMD = [0x03, TERM]
START_CMD = [0x04, TERM] # Start from row 0.  If current row is hold forever use to restart pattern.
...
class Colours(enum.Enum):
    ...
    RED        = [0xFF,0x00,0x00]
    AMBER      = [0xFF,60,0x00]
    YELLOW     = [0xFF,125,0x00]
    GREEN      = [0x00,0xFF,0x00]
    GREEN_DIM  = [0x00,0x10,0x00]
    GREEN_BLUE = [0x00,0xFF,60]
    BLUE_GREEN = [0x00,0xFF,180]
    BLUE       = [0x00,0x00,0xFF]
    BLUE_DIM   = [0x00,0x00,10]
    PURPLE     = [0xFF,0x00,40]
    WHITE      = [0xFF,0xFF,0xFF]

RAINBOW = Colours.RED.value + Colours.AMBER.value + Colours.YELLOW.value + \
          Colours.GREEN.value + Colours.GREEN_BLUE.value + Colours.BLUE_GREEN.value + \
          Colours.BLUE.value + Colours.PURPLE.value + Colours.WHITE.value
...
def read_args():
    ...
    """.format(os.path.basename(__file__),
                 '\n        '.join(NON_COLOUR_PATTERNS.keys()),
                 '\n        '.join(COLOUR_PATTERNS.keys()),
                 '\n        '.join(all_colours)))
    ...
    for opt, arg in opts:
        ...
        if opt == '-p':
            ...
            if not my_pattern in all_patterns:
                print("\nERROR: Pattern not recognised. pattern={}".format(my_pattern))
                print(help_string)
                sys.exit()
        if opt == '-c':
            my_colour = arg.upper()
...
def serial_read_handler(ser):
    ...
    while not STOP_THREAD.isSet():
        ...
        if reading.hex() != '':
            ...
            if RX_QUEUE.full():
                LOGGER.error("*** DEBUG: rxQueue is full.  Dumping oldest message")
                RX_QUEUE.get()
            RX_QUEUE.put(reading)

            my_time = datetime.datetime.now().strftime("%H:%M:%S.%f")
            LOGGER.debug("DEBUG RX: {},  {}".format(my_time, reading.hex()))
...
def serial_write_handler(ser):
    ...
    while not STOP_THREAD.isSet():
        try:
            ...
            my_time = datetime.datetime.now().strftime("%H:%M:%S.%f")
            my_print_message = ["0x{:02x}".format(x) for x in my_message]
            LOGGER.debug("DEBUG Tx: {},  {}".format(my_time, my_print_message))
...
def start_serial_threads(port, baud):
    ...
    try:
        ...
        LOGGER.error('Error opening port.{}'.format(err))
        sys.exit()
    LOGGER.info("Serial port opened: {}".format(port))
...
def flush_rx_queue():
    ...
    while not RX_QUEUE.empty():
        msg = RX_QUEUE.get()
        LOGGER.debug("Dumping={}".format(msg))
def send_command(cmd, resp_value_expected=False, timeout=2):
    # pylint: disable=too-many-branches, too-many-statements, too-many-nested-blocks
    ''' resp_value_expected defines whether or no we expect a value in the response (2nd Byte)
        currently only the software version command gives a response value
    ...
    while True:
        ...
        if state == "sendCmd":
            ...
            if try_count < 3:
                ...
                resp_value = "TIMEOUT: 3x retries while waiting for correct response"
                state = 'return'
...
def send_led_cmds(cmds):
    ...
    '''
    for cmd in cmds:
        resp, resp_state, resp_value = send_command(cmd, resp_value_expected=False)

        # Return early if any one of the commands fails
        if not resp_state:
            LOGGER.debug("Cmd failed: cmd=%s, resp=%s, resp_value=%s", cmd, resp, resp_value)
...
def time_to_bytes(my_time):
    ...
    '''
    my_time_hex = "{:04x}".format(my_time)
...
def chase_me(colour, direction=True):
    ...
    for i in range(0, 9):
        if direction:
            i = -i
        row = build_led_row_cmd(led_rotate_pattern(leds, i*3), fade_time, hold_time)
        rows.append(row)
    return rows
def build_led_row_cmd(row_pattern, fade_time=0, hold_time=HOLD_FOREVER):
    ...
    """
    row = LED_CMD + row_pattern + time_to_bytes(fade_time) + time_to_bytes(hold_time) + [TERM]
    return row
...
def strobe(colour):
    ...
    row_1 = build_led_row_cmd(leds_all_same_colour(colour.value), fade_time, hold_time)
    row_2 = build_led_row_cmd(leds_all_same_colour(Colours.OFF.value), fade_time, hold_time)
    row_3 = build_led_row_cmd(leds_all_same_colour(colour.value), fade_time, hold_time)
    row_4 = build_led_row_cmd(leds_all_same_colour(Colours.OFF.value), fade_time, final_hold_time)
...
def breathing(colour):
    ...
    row_1 = build_led_row_cmd(leds_all_same_colour(my_led_dim), fade_time, hold_time)
    row_2 = build_led_row_cmd(leds_all_same_colour(colour.value), fade_time, hold_time)
    rows = [row_1, row_2]
    return rows
def chase_cw(colour):
...
def clock(colour):
    ...
    hold_time = 500
    for i in range(0, 8):
        rows.append(build_led_row_cmd(led_right_shift_pattern(pattern, i), fade_time, hold_time))
...
def rgb_funk():
    ...
    """
    row_1_leds = Colours.RED.value*3 + Colours.GREEN.value*3 + Colours.BLUE.value*3
    row_2_leds = Colours.GREEN.value*3 + Colours.BLUE.value*3 + Colours.RED.value*3
    row_3_leds = Colours.BLUE.value*3 + Colours.RED.value*3 + Colours.GREEN.value*3
...
def rgb_rotate():
    ...
    hold_time = 200
    for i in range(0, 9):
        rows.append(build_led_row_cmd(led_rotate_pattern(leds, i*3), fade_time, hold_time))
    return rows
def all_on_funk(pattern=None):
    ...
    rows = []
    for i in range(0, 9):
        rows.append(build_led_row_cmd(led_rotate_pattern(pattern, i*-3), fade_time, hold_time))
    return rows
...
def unit_test():
    ...
    if resp_state:
        print("Version={}".format(resp_value))
    else:
        print("Error: resp={}".format(resp_value))

    for pattern in COLOUR_PATTERNS:
        for colour in Colours:
            print("Pattern={}, Colour={}".format(pattern, colour.name))
...
def show_pattern(pattern, colour, debug=False):
    ...
    if pattern in COLOUR_PATTERNS and colour is None:
        LOGGER.error(("Colour pattern specified but no colour given.  "
                      "pattern=%s, colour=%s", pattern, colour))
        return False

    if pattern in NON_COLOUR_PATTERNS and colour:
        LOGGER.info(("Non-Colour pattern specified but a colour was specified.  "
                     "Colour was ignored.  pattern=%s, colour=%s", pattern, colour))
