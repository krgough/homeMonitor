...
Synchronous version of HiveAuth

KG Modification Notes:

- added logging to the exception handling
- Removed import of external exceptions from other modules
- Removed the use of socket to generate a device name
- Added list devices to class
...
class HiveAuth:
    ...
    def __init__(
        ...
        self.__pool_id = self.data.get("UPID").split("_")[1]
    ...
    def get_password_authentication_key(
        ...
        username_password = f"{self.__pool_id}{username}:{password}"
    ...
    def process_challenge(self, challenge_parameters: dict):
        ...
        msg = (
            ...
            bytearray(self.__pool_id, "utf-8")
    ...
    def login(self):
        ...
        LOGGER.error("Challenge %s is not supported", challenge_name)
    ...
    def device_login(self):
        ...
        if login_result.get("ChallengeName") == self.DEVICE_VERIFIER_CHALLENGE:
            ...
            LOGGER.error("Invalid device authentication")
            raise NotImplementedError
    ...
    def device_registration(self, device_name: str = None):
        """Register Device."""
        creds = self.confirm_device(device_name)
